...
// PUT /api/ic-sessions/:id — Admin or Deal Lead only
router.put("/:id", requireRole("Admin", "Deal Lead"), async (req, res) => {
  ...
  const { sessionDate, attendees, outcome, conditions, notes } = req.body as {
    sessionDate?: string; attendees?: string | null; outcome?: string;
    conditions?: string | null; notes?: string | null;
  };
  const [updated] = await db
    .update(icSessionsTable)
    .set({
    ...
    .where(eq(icSessionsTable.id, id))
    .returning();
