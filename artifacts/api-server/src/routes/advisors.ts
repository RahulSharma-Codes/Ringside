...
router.put("/:id", async (req, res) => {
  ...
  const [existing] = await db.select().from(dealAdvisorsTable).where(eq(dealAdvisorsTable.id, id)).limit(1);
  if (!existing) return res.status(404).json({ error: "Advisor not found" });
  if (!(await canAccessTarget(req, existing.targetId))) return res.status(404).json({ error: "Advisor not found" });
...
router.get("/:id/conflict-notes", async (req, res) => {
  ...
  if (!(await canAccessTarget(req, advisor.targetId))) return res.status(404).json({ error: "Advisor not found" });
